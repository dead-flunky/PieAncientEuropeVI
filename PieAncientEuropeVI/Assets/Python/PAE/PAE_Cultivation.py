    ...
    for pCity in lCities:
        if eBonus in L.LBonusCultivatableCoast:
            ...
            if not isCityHasBonus(pCity, eBonus):
                ...
                return True
        # Pro Stadt jedes Gut erlaubt
        elif not isCityHasBonus(pCity, eBonus):
            return True
        # Pro Stadtstufe eine bestimmte Anzahl bestimmter Typen (Vieh oder Getreide) erlaubt
            ...
            if eBonusOnBoard in L.LBonusCultivatableCoast:
                ...
                if not isCityHasBonus(loopCity, eBonusOnBoard):
                    if iDistance == 0:
                        ...
                        iValue = 1/iDistance
            # Jedes Bonusgut darf 1x pro Stadt verbreitet werden
            elif not isCityHasBonus(loopCity, eBonusOnBoard):
                if iDistance == 0:
                    iValue = 2
                else:
                    iValue = 1/iDistance
            # Nur ein Gut pro Getreide/Vieh darf verbreitet werden
