...
class CvPediaReligion:
    ...
    def interfaceScreen(self, iReligion):
        ...
        screen.setText(self.top.getNextWidgetName(), "Background", self.top.MENU_TEXT, CvUtil.FONT_LEFT_JUSTIFY, self.top.X_MENU, self.top.Y_MENU,
                       0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_PEDIA_MAIN, CivilopediaPageTypes.CIVILOPEDIA_PAGE_RELIGION, -1)
    ...
    # PAE: can be found by following CIVs
    def placeCanFound(self):

        screen = self.top.getScreen()

        panelName = self.top.getNextWidgetName()
        screen.addPanel(panelName, localText.getText("TXT_KEY_PEDIA_CANFOUND", ()), "", False, True, self.X_BUILDING_PANE,
                        self.Y_BUILDING_PANE + 110, self.W_BUILDING_PANE, self.H_BUILDING_PANE, PanelStyles.PANEL_STYLE_BLUE50)
        ...
        for iCiv in range(gc.getNumCivilizationInfos()):
            ...
            iTech = gc.getReligionInfo(self.iReligion).getTechPrereq()
            if not pCiv.isCivilizationDisableTechs(iTech):
                screen.attachImageButton(panelName, "", pCiv.getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_CIV, iCiv, 1, False)
