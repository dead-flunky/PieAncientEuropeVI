...
class CvPediaUnitChart:
    ...
    def interfaceScreen(self, iGroup):
        ...
        screen.setText(self.top.getNextWidgetName(), "Background", self.top.MENU_TEXT, CvUtil.FONT_LEFT_JUSTIFY, self.top.X_MENU, self.top.Y_MENU,
                       0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_PEDIA_MAIN, CivilopediaPageTypes.CIVILOPEDIA_PAGE_UNIT_GROUP, -1)
...
#		screen.attachTableControlGFC( panelName, szTable, 4, False, True, 32, 32, TableStyles.TABLE_STYLE_EMPTY );
    ...
    def placeLinks(self, bRedraw):
        ...
        iSelected = 0
        if bRedraw:
            screen.appendListBoxString(self.top.LIST_ID, localText.getText("TXT_KEY_PEDIA_ALL_GROUPS", ()),
                                       WidgetTypes.WIDGET_PEDIA_JUMP_TO_UNIT_COMBAT, gc.getNumUnitCombatInfos(), 0, CvUtil.FONT_LEFT_JUSTIFY)
        if self.iGroup == gc.getNumUnitCombatInfos():
            iSelected = 0
