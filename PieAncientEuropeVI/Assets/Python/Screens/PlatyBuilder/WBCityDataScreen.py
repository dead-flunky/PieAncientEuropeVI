...
class WBCityDataScreen:
    ...
    def interfaceScreen(self, pCityX):
        ...
        screen.setText("CityDataExit", "Background", "<font=4>" + CyTranslator().getText("TXT_KEY_PEDIA_SCREEN_EXIT", ()).upper() + "</font>", CvUtil.FONT_RIGHT_JUSTIFY,
        ...
        screen.setText("SpecialistAll", "Background",  sText, CvUtil.FONT_RIGHT_JUSTIFY, screen.getXResolution() /
                       2 - 20, self.iTable_Y - 30, -0.1, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
        screen.setText("GreatPeopleProgressAll", "Background", sText, CvUtil.FONT_RIGHT_JUSTIFY, screen.getXResolution()
                       * 3/4 - 20, self.iTable_Y - 30, -0.1, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
    ...
    def sortBuildings(self):
        ...
        for i in xrange(gc.getNumBuildingClassInfos()):
            ...
            if bWonder and not isLimitedWonderClass(i):
                continue
            if not bWonder and isLimitedWonderClass(i):
                continue
    ...
    def placeModify(self):
        ...
        screen.setLabel("ModifyHeader", "Background", "<font=3b>" + CyTranslator().getText("TXT_KEY_WB_MODIFY", (sText,)) + "</font>",
                        CvUtil.FONT_CENTER_JUSTIFY, iX + iTableWidth/2, iY, -0.1, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
        ...
        for i in xrange(CommerceTypes.NUM_COMMERCE_TYPES):
            screen.addPullDownString("YieldType", gc.getCommerceInfo(i).getDescription(), i + YieldTypes.NUM_YIELD_TYPES,
                                     i + YieldTypes.NUM_YIELD_TYPES, iSelectedYield == i + YieldTypes.NUM_YIELD_TYPES)
    ...
    def placeSpecialist(self):
        ...
        screen.setLabel("SpecialistHeader", "Background", u"<font=4b>" + CyTranslator().getText("TXT_KEY_WB_FREE_SPECIALISTS", ()) + "</font>",
                        CvUtil.FONT_CENTER_JUSTIFY, iX + iWidth/2, self.iTable_Y - 60, -0.1, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
    ...
    def placeGreatPeople(self):
        ...
        screen.setLabel("GreatPeopleHeader", "Background", u"<font=4b>" + CyTranslator().getText("TXT_KEY_CONCEPT_GREAT_PEOPLE", ()) + "</font>",
                        CvUtil.FONT_CENTER_JUSTIFY, iX + iWidth/2, self.iTable_Y - 60, -0.1, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
    ...
    def placeBonus(self):
        ...
        screen.setLabel("BonusHeader", "Background", u"<font=4b>" + CyTranslator().getText("TXT_KEY_PEDIA_CATEGORY_BONUS", ()) + "</font>",
                        CvUtil.FONT_CENTER_JUSTIFY, iX + iWidth/2, self.iTable_Y - 60, -0.1, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
        ...
        for item in lBonus:
            ...
            if pCity.isNoBonus(item[1]):
                screen.setTableText("WBBonus", 0, iRow, "", CyArtFileMgr().getInterfaceArtInfo("INTERFACE_BUTTONS_CANCEL").getPath(),
                                    WidgetTypes.WIDGET_PYTHON, 1030, item[1], CvUtil.FONT_LEFT_JUSTIFY)
            else:
            ...
            screen.setTableText("WBBonus", 1, iRow, "<font=3>" + sColor + sItem + "</font></color>",
                                gc.getBonusInfo(item[1]).getButton(), WidgetTypes.WIDGET_PYTHON, 7878, item[1], CvUtil.FONT_LEFT_JUSTIFY)
