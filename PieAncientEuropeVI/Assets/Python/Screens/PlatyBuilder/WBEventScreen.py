...
class WBEventScreen:
    ...
    def placeEventPlayers(self):
        ...
        for iPlayerX in xrange(gc.getMAX_CIV_PLAYERS()):
            ...
            if pPlayerX.isAlive():
                ...
                screen.setTableText("WBEventPlayer", 0, iRow, "", gc.getCivilizationInfo(iCivilization).getButton(),
                                    WidgetTypes.WIDGET_PYTHON, 7872, iPlayerX * 10000 + iCivilization, CvUtil.FONT_LEFT_JUSTIFY)
                screen.setTableText("WBEventPlayer", 1, iRow, "<font=3>" + sColor + pPlayerX.getName() + "</font></color>",
                                    gc.getLeaderHeadInfo(iLeader).getButton(), WidgetTypes.WIDGET_PYTHON, 7876, iPlayerX * 10000 + iLeader, CvUtil.FONT_LEFT_JUSTIFY)

        screen.setLabel("EventPlayerText", "Background", "<font=3b>" + sHeader + "</font>", CvUtil.FONT_CENTER_JUSTIFY,
                        screen.getXResolution() * 3/10, self.iTable_Y - 30, -0.1, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
    ...
    def placeUnits(self):
        ...
        for i in xrange(pPlot.getNumUnits()):
            ...
            if pUnitX.getID() == iSelectedUnit:
                sColor = CyTranslator().getText("[COLOR_POSITIVE_TEXT]", ())
                sHeader = sText
            screen.setTableText("WBEventUnit", 2, iRow, "<font=3>" + sColor + sText + "</font></color>", pUnitX.getButton(),
        ...
        screen.setLabel("EventUnitText", "Background", "<font=3b>" + sHeader + "</font>", CvUtil.FONT_CENTER_JUSTIFY,
                        screen.getXResolution() * 9/10, self.iTable_Y - 30, -0.1, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
    ...
    def placeCorporations(self):
        ...
        for item in lCorporations:
            ...
            if item[1] == iSelectedCorporation:
                sColor = CyTranslator().getText("[COLOR_POSITIVE_TEXT]", ())
                sHeader = item[0]
            screen.setTableText("WBEventCorporation", 0, iRow, "<font=3>" + sColor + item[0] + "</font></color>",
                                gc.getCorporationInfo(item[1]).getButton(), WidgetTypes.WIDGET_PYTHON, 8201, item[1], CvUtil.FONT_LEFT_JUSTIFY)
        screen.setLabel("EventCorporationText", "Background", "<font=3b>" + sHeader + "</font>", CvUtil.FONT_CENTER_JUSTIFY,
                        screen.getXResolution() * 7/10, screen.getYResolution()/2, -0.1, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
    ...
    def placeReligions(self):
        ...
        for item in lReligions:
            ...
            if item[1] == iSelectedReligion:
                sColor = CyTranslator().getText("[COLOR_POSITIVE_TEXT]", ())
                sHeader = item[0]
            screen.setTableText("WBEventReligion", 0, iRow, "<font=3>" + sColor + item[0] + "</font></color>",
                                gc.getReligionInfo(item[1]).getButton(), WidgetTypes.WIDGET_HELP_RELIGION, item[1], -1, CvUtil.FONT_LEFT_JUSTIFY)
        screen.setLabel("EventReligionText", "Background", "<font=3b>" + sHeader + "</font>", CvUtil.FONT_CENTER_JUSTIFY,
                        screen.getXResolution() / 2, screen.getYResolution()/2, -0.1, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
    ...
    def placeBuildings(self):
        ...
        for item in lBuildings:
            ...
            if item[1] == iSelectedBuilding:
                sColor = CyTranslator().getText("[COLOR_POSITIVE_TEXT]", ())
                sHeader = item[0]
            screen.setTableText("WBEventBuilding", 0, iRow, "<font=3>" + sColor + item[0] + "</font></color>",
                                gc.getBuildingInfo(item[1]).getButton(), WidgetTypes.WIDGET_HELP_BUILDING, item[1], -1, CvUtil.FONT_LEFT_JUSTIFY)
        screen.setLabel("EventBuildingText", "Background", "<font=3b>" + sHeader + "</font>", CvUtil.FONT_CENTER_JUSTIFY, screen.getXResolution()
                        * 3/10, screen.getYResolution()/2, -0.1, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
    ...
    def placeOtherCities(self):
        ...
        while loopCity:
            if not loopCity.isNone() and loopCity.getOwner() == pPlayer.getID():  # only valid cities
                ...
                screen.setTableText("WBOtherCity", 0, iRow, "<font=3>" + sColor + loopCity.getName() + "</font></color>", "",
                                    WidgetTypes.WIDGET_PYTHON, 7200 + iOtherPlayer, loopCity.getID(), CvUtil.FONT_LEFT_JUSTIFY)
            (loopCity, pIter) = pPlayer.nextCity(pIter, False)
        screen.setLabel("OtherCityText", "Background", "<font=3b>" + sHeader + "</font>", CvUtil.FONT_CENTER_JUSTIFY,
                        screen.getXResolution() * 7/10, self.iTable_Y - 30, -0.1, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
    ...
    def placeOtherPlayers(self):
        ...
        for iPlayerX in xrange(gc.getMAX_CIV_PLAYERS()):
            ...
            if pPlayerX.isAlive():
                ...
                iCivilization = pPlayerX.getCivilizationType()
                iLeader = pPlayerX.getLeaderType()
                screen.setTableText("WBOtherPlayer", 0, iRow, "", gc.getCivilizationInfo(iCivilization).getButton(),
                                    WidgetTypes.WIDGET_PYTHON, 7872, iPlayerX * 10000 + iCivilization, CvUtil.FONT_LEFT_JUSTIFY)
                screen.setTableText("WBOtherPlayer", 1, iRow, "<font=3>" + sColor + pPlayerX.getName() + "</font></color>",
                                    gc.getLeaderHeadInfo(iLeader).getButton(), WidgetTypes.WIDGET_PYTHON, 7876, iPlayerX * 10000 + iLeader, CvUtil.FONT_LEFT_JUSTIFY)

        screen.setLabel("OtherPlayerText", "Background", "<font=3b>" + sHeader + "</font>", CvUtil.FONT_CENTER_JUSTIFY,
                        screen.getXResolution()/2, self.iTable_Y - 30, -0.1, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
    ...
    def placeEvents(self):
        ...
        screen.setLabel("EventText", "Background", "<font=3b>" + sHeader + "</font>", CvUtil.FONT_CENTER_JUSTIFY,
                        screen.getXResolution() / 10, self.iTable_Y - 30, -0.1, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
    ...
    def handleInput(self, inputClass):
        ...
        if inputClass.getFunctionName() == "CurrentPage":
            ...
            triggerData = pPlayer.initTriggeredData(iSelectedEvent, True, iCity, pPlot.getX(), pPlot.getY(), iOtherPlayer, iOtherCity,
                                                    iSelectedReligion, iSelectedCorporation, iSelectedUnit, iSelectedBuilding)
            screen.hideScreen()
        return 1
