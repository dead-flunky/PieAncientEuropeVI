...
from CvPythonExtensions import (CyGlobalContext, CyArtFileMgr, CyTranslator,
																...
																TradeData, TradeableItems, PopupStates,
																CyGame, CommerceTypes, MouseFlags, TableStyles,
																ButtonStyles, GenericButtonSizes, DenialTypes,
		...
		def drawContents(self, bInitial):
				...
				for i in range(len(self.ORDER_LIST)):
						...
						if (self.iScreen != self.SCREEN_DICT[szScreen]):
								screen.setText(szTextId, "", u"<font=4>" + localText.getText(self.TXT_KEY_DICT[szScreen], ()).upper() + u"</font>", CvUtil.FONT_CENTER_JUSTIFY,
															 xLink, self.Y_LINK, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_FOREIGN_ADVISOR, self.SCREEN_DICT[szScreen], -1)
						else:
								screen.setText(szTextId, "", u"<font=4>" + localText.getColorText(self.TXT_KEY_DICT[szScreen], (), gc.getInfoTypeForString("COLOR_YELLOW")).upper(
								) + u"</font>", CvUtil.FONT_CENTER_JUSTIFY, xLink, self.Y_LINK, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_FOREIGN_ADVISOR, -1, -1)
						xLink += self.DX_LINK
		...
		def drawInfo(self, bInitial):
				...
				for iLoopPlayer in range(gc.getMAX_PLAYERS()):
						...
						if (gc.getPlayer(iLoopPlayer).isAlive() and iLoopPlayer != self.iActiveLeader and (gc.getTeam(gc.getPlayer(iLoopPlayer).getTeam()).isHasMet(gc.getPlayer(self.iActiveLeader).getTeam()) or gc.getGame().isDebugMode()) and not gc.getPlayer(iLoopPlayer).isBarbarian() and not gc.getPlayer(iLoopPlayer).isMinorCiv()):
								...
								religionName = self.getNextWidgetName()  # noqa
								szPlayerReligion = ""
								...
								screen.attachTextGFC(infoPanelName, "", localText.getText("TXT_KEY_FOREIGN_ADVISOR_TRADE", (self.calculateTrade(
										self.iActiveLeader, iLoopPlayer), )), FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
...
#     ExoticForPrint ("iLoopPlayer = %d" % iLoopPlayer)
						...
						screen.attachCheckBoxGFC(playerPanelName, szName, gc.getLeaderHeadInfo(gc.getPlayer(iLoopPlayer).getLeaderType()).getButton(), ArtFileMgr.getInterfaceArtInfo(
								"BUTTON_HILITE_SQUARE").getPath(), self.GLANCE_BUTTON_SIZE, self.GLANCE_BUTTON_SIZE, WidgetTypes.WIDGET_LEADERHEAD, iLoopPlayer, self.iActiveLeader, ButtonStyles.BUTTON_STYLE_LABEL)
								...
								if self.ltPlayerMet[j]:
										if j != self.iActiveLeader:  # TODO: Wrong selection, too.
												...
												if nAttitude is not None:
														...
														screen.setTextAt(szName, playerPanelName, szText, CvUtil.FONT_CENTER_JUSTIFY, self.X_GLANCE_OFFSET + (self.nSpread * nCount),
																						 self.Y_GLANCE_OFFSET + (self.GLANCE_BUTTON_SIZE / 4), -0.1, FontTypes.GAME_FONT, WidgetTypes.WIDGET_LEADERHEAD, j, iLoopPlayer)
												else:
														szText = localText.getText("TXT_KEY_FOREIGN_ADVISOR_NONE", ())
														screen.setTextAt(szName, playerPanelName, szText, CvUtil.FONT_CENTER_JUSTIFY, self.X_GLANCE_OFFSET + (self.nSpread * nCount),
																						 self.Y_GLANCE_OFFSET + (self.GLANCE_BUTTON_SIZE / 4), -0.1, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
										nCount += 1
		...
		def getAttitudeText(self, nAttitude, nPlayer, nTarget):
				...
				iAnzCivs = len(self.ltPlayerRelations)
				if iAnzCivs < 9: szText = "<font=4>" + szText
				elif iAnzCivs < 14: szText = "<font=3>" + szText
				else: szText = "<font=2>" + szText
				szText += "</font>"
				return szText
...
#     self.RES_TOP_BOTTOM_SPACE = self.MIN_TOP_BOTTOM_SPACE
		...
		def drawResourceDeals(self, bInitial):
				...
				screen.addMultiListControlGFC(self.availableMultiList, "", self.SURPLUS_ICONS_X, self.SURPLUS_ICONS_Y, self.RESOURCE_ICON_SIZE *
																			len(listSurplus), self.RESOURCE_ICON_SIZE, 1, 32, 32, TableStyles.TABLE_STYLE_EMPTY)

				self.availableTable = self.getNextWidgetName()
				# add the circles behind the amounts
				if (self.RES_SHOW_SURPLUS_AMOUNT_ON_TOP):
						for iIndex in range(len(listSurplus)):
								screen.addDDSGFC(self.availableTable + "Circle" + str(iIndex), ArtFileMgr.getInterfaceArtInfo("WHITE_CIRCLE_40").getPath(),
																 self.SURPLUS_CIRCLE_X_START + iIndex * self.RESOURCE_ICON_SIZE, self.SURPLUS_CIRCLE_Y, 16, 16, WidgetTypes.WIDGET_GENERAL, -1, -1)

				# add the table showing the amounts
				screen.addTableControlGFC(self.availableTable, len(listSurplus), self.SURPLUS_TABLE_X, self.SURPLUS_TABLE_Y, len(listSurplus) *
																	self.RESOURCE_ICON_SIZE, self.TABLE_CONTROL_HEIGHT, False, False, 16, 16, TableStyles.TABLE_STYLE_EMPTY)
				...
				for iIndex in range(len(listSurplus)):
						...
						screen.appendMultiListButton(self.availableMultiList, gc.getBonusInfo(listSurplus[iIndex]).getButton(), 0, WidgetTypes.WIDGET_PEDIA_JUMP_TO_BONUS, listSurplus[iIndex], -1, False)
...
#                         , WidgetTypes.WIDGET_PEDIA_JUMP_TO_BONUS, iLoopBonus )
		...
		def initTechTable(self):
				...
				columns = (IconGrid.GRID_ICON_COLUMN, IconGrid.GRID_MULTI_LIST_COLUMN, IconGrid.GRID_MULTI_LIST_COLUMN, IconGrid.GRID_TEXT_COLUMN,
