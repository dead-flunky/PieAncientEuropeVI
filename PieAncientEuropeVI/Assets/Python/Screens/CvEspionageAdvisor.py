...
class CvEspionageAdvisor:
    ...
    def drawContents(self):
        ...
        screen.setLabel(self.szMakingText, "Background", szText, CvUtil.FONT_LEFT_JUSTIFY, self.X_MAKING_TEXT,
                        self.Y_MAKING_TEXT, self.Z_CONTROLS, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
        ...
        if (self.iTargetPlayer != -1):
            ...
            screen.setLabel(self.szCitiesTitleText, "Background", szText, CvUtil.FONT_LEFT_JUSTIFY, self.X_CITY_LIST,
                            self.Y_CITY_LIST - 40, self.Z_CONTROLS, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)

            self.szEffectsTitleText = "EffectsTitle"
            szText = u"<font=4>" + localText.getText("TXT_KEY_ESPIONAGE_SCREEN_PASSIVE_EFFECTS", ()) + "</font>"
            screen.setLabel(self.szEffectsTitleText, "Background", szText, CvUtil.FONT_LEFT_JUSTIFY, self.X_EFFECTS_LIST,
                            self.Y_EFFECTS_LIST - 40, self.Z_CONTROLS, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)

            self.szMissionsTitleText = "MissionsTitle"
            szText = u"<font=4>" + localText.getText("TXT_KEY_ESPIONAGE_SCREEN_MISSIONS", ()) + "</font>"
            screen.setLabel(self.szMissionsTitleText, "Background", szText, CvUtil.FONT_LEFT_JUSTIFY, self.X_MISSIONS_LIST,
                            self.Y_MISSIONS_LIST - 40, self.Z_CONTROLS, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)

            self.szEffectsCostTitleText = "EffectsCostTitle"
            szText = u"<font=4>" + localText.getText("TXT_KEY_ESPIONAGE_SCREEN_COST", ()) + "</font>"
            screen.setLabel(self.szEffectsCostTitleText, "Background", szText, CvUtil.FONT_LEFT_JUSTIFY, self.X_EFFECTS_COSTS_LIST,
                            self.Y_EFFECTS_COSTS_LIST - 40, self.Z_CONTROLS, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)

            self.szMissionsCostTitleText = "MissionsCostTitle"
            szText = u"<font=4>" + localText.getText("TXT_KEY_ESPIONAGE_SCREEN_COST", ()) + "</font>"
            screen.setLabel(self.szMissionsCostTitleText, "Background", szText, CvUtil.FONT_LEFT_JUSTIFY, self.X_MISSIONS_COSTS_LIST,
                            self.Y_MISSIONS_COSTS_LIST - 40, self.Z_CONTROLS, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
            ...
            for iPlayerID in self.aiKnownPlayers:
                ...
                szTempBuffer = u"<color=%d,%d,%d,%d>%s (%s)</color>" % (pTargetPlayer.getPlayerTextColorR(), pTargetPlayer.getPlayerTextColorG(),
                                                                        pTargetPlayer.getPlayerTextColorB(), pTargetPlayer.getPlayerTextColorA(), pTargetPlayer.getName(), self.getMultiplierAgainstTarget(iPlayerID))
                ...
                screen.setImageButtonAt(szName, attach, ArtFileMgr.getInterfaceArtInfo("INTERFACE_BUTTONS_PLUS").getPath(), 45,
                                        iY + 1, iSize, iSize, WidgetTypes.WIDGET_GENERAL, self.iIncreaseButtonID, iPlayerID)
                self.iDecreaseButtonID = 556
                szName = "DecreaseButton%d" % (iPlayerID)
                self.aszDecreaseButtons.append(szName)
                screen.setImageButtonAt(szName, attach, ArtFileMgr.getInterfaceArtInfo("INTERFACE_BUTTONS_MINUS").getPath(),
                                        60, iY + 1, iSize, iSize, WidgetTypes.WIDGET_GENERAL, self.iDecreaseButtonID, iPlayerID)
    ...
    def refreshScreen(self):
        ...
        if (self.iTargetPlayer != -1):
            ...
            if (self.iTargetPlayer != -1):
                ...
                screen.addTableControlGFCWithHelp(szEffectsTable, 4, self.X_EFFECTS_LIST, self.Y_EFFECTS_LIST, self.W_EFFECTS_LIST + self.W_EFFECTS_COSTS_LIST +
                                                  self.W_TABLE_1 + self.W_TABLE_3, self.H_EFFECTS_LIST, False, False, 32, 32, TableStyles.TABLE_STYLE_STANDARD, szHelpText)
                ...
                screen.addTableControlGFCWithHelp(szMissionsTable, 4, self.X_MISSIONS_LIST, self.Y_MISSIONS_LIST, self.W_MISSIONS_LIST + self.W_MISSIONS_COSTS_LIST +
                                                  self.W_TABLE_1 + self.W_TABLE_3, self.H_MISSIONS_LIST, False, False, 32, 32, TableStyles.TABLE_STYLE_STANDARD, szHelpText)
        ...
        if (self.iTargetPlayer != -1):
            ...
            if (self.iTargetPlayer != -1):
                ...
                if (inputClass.getData1() == self.iIncreaseButtonID):
                    ...
                    screen.setLabelAt("AmountText%d" % (iPlayerID), "LeaderContainer%d" % (iPlayerID), szText, 0, 247,
                                      15 - 1, self.Z_CONTROLS, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
