...
##############################################################################
## Version History
## PAE - Added Dense Forest
## PAE - Added savanna
## PAE - Added feature swamp. - PAE V Patch 4 changed to terrain swamp
## PAE - more gates between peaks, check border coast peaks
##
...
#Lower than leafy is jungle and/or Savanna and Dense Forest (PAE)
DenseForestAltitude = .75
SavannaAltitude = .15
LeafyAltitude = .30
...
##  pref.needCoastalStart = False
		...
		pref = CivPreference(GetInfoType("CIVILIZATION_EGYPT"))
		pref.idealMoisture = 0.2
		pref.idealAltitude = 0.25
		pref.altitudeWeight = 0.25
		...
		pref = CivPreference(GetInfoType("CIVILIZATION_ASSYRIA"))
		pref.idealMoisture = 0.3
		pref.idealAltitude = 0.25
		pref.altitudeWeight = 0.25
		pref.distanceWeight = 0.25
		pref.needCoastalStart = False
		...
		pref = CivPreference(GetInfoType("CIVILIZATION_BABYLON"))
		pref.idealMoisture = 0.2
		pref.idealAltitude = 0.25
		...
		pref = CivPreference(GetInfoType("CIVILIZATION_BERBER"))
		pref.idealMoisture = 0.1
		pref.idealAltitude = 0.25
		pref.altitudeWeight = 2.0
		pref.distanceWeight = 0.75
		...
		pref = CivPreference(GetInfoType("CIVILIZATION_SUMERIA"))
		pref.idealMoisture = 0.2
		pref.idealAltitude = 0.25
		pref.altitudeWeight = 2.0
		pref.distanceWeight = 0.75
		...
		pref = CivPreference(GetInfoType("CIVILIZATION_NUBIA"))
		pref.idealMoisture = 0.2
		pref.idealAltitude = 0.25
		pref.altitudeWeight = 2.0
		pref.distanceWeight = 0.75
		...
		pref = CivPreference(GetInfoType("CIVILIZATION_NUMIDIA"))
		pref.idealMoisture = 0.2
		pref.idealAltitude = 0.25
		pref.altitudeWeight = 2.0
		pref.distanceWeight = 0.75
		...
		pref = CivPreference(GetInfoType("CIVILIZATION_PHON"))
		...
		pref = CivPreference(GetInfoType("CIVILIZATION_CARTHAGE"))
		pref.idealMoisture = 0.35
		...
		pref = CivPreference(GetInfoType("CIVILIZATION_ETRUSCANS"))
		pref.idealMoisture = 0.35
		pref.idealAltitude = 0.45
		pref.altitudeWeight = 2.0
		pref.distanceWeight = 0.75
		pref.needCoastalStart = True
		pref.allowForestStart = True
		...
		pref = CivPreference(GetInfoType("CIVILIZATION_CELT"))
		pref.idealMoisture = 0.85
		pref.idealAltitude = 0.85
		pref.altitudeWeight = 2.0
		pref.distanceWeight = 0.75
		pref.needCoastalStart = False
		pref.allowForestStart = True
		...
		pref = CivPreference(GetInfoType("CIVILIZATION_GALLIEN"))
		pref.idealMoisture = 0.85
		pref.idealAltitude = 0.85
		pref.altitudeWeight = 2.0
		pref.distanceWeight = 0.75
		...
		pref = CivPreference(GetInfoType("CIVILIZATION_GERMANEN"))
		pref.idealMoisture = 0.85
		pref.idealAltitude = 0.85
		pref.altitudeWeight = 2.0
		pref.distanceWeight = 0.75
		...
		pref = CivPreference(GetInfoType("CIVILIZATION_GREECE"))
		pref.idealMoisture = 0.45
		pref.idealAltitude = 0.45
		pref.altitudeWeight = 2.0
		pref.distanceWeight = 0.75
		pref.needCoastalStart = True
		...
		pref = CivPreference(GetInfoType("CIVILIZATION_ATHENS"))
		pref.idealMoisture = 0.45
		pref.idealAltitude = 0.45
		pref.altitudeWeight = 2.0
		pref.distanceWeight = 0.75
...
class PythonRandom :
		...
		def seed(self):
				...
				if UsePythonRandom:
						...
						self.usePR = False
				if self.usePR and CyGame().isGameMultiPlayer():
								...
								if plotMap.plotMap[i] != plotMap.OCEAN:
										...
										if rainFall < DesertThreshold:
												if rainFall < ((PRand.random()*DesertThreshold)/2.0) + (DesertThreshold/2.0):
														# PAE desert gets some plains
														if PRand.randint(0,6) < 5:
												...
												if rainFall < ((PRand.random() * (PlainsThreshold - DesertThreshold))/2.0) + DesertThreshold + ((PlainsThreshold - DesertThreshold)/2.0):
														self.terrainMap[i] = self.PLAINS
												else:
														self.terrainMap[i] = self.GRASS
										else:
		...
		def assignStartingPlots(self):
				...
				for playerIndex in playerList:
						...
						if bestRegion != None:
							...
							if startPlot != None: player.setStartingPlot(startPlot,True)
...
##            {"r":startRegion.region.ID,"ad":altitudeDiff,"md":moistureDiff,"aw":civPref.altitudeWeight,"wd":weightedAverageDiff}
				...
				if len(bestRegionList):
					...
					return startRegion
				elif len(availableRegionList): return availableRegionList[0]
		...
		def collectAllWatchtowers(self):
				...
				for y in range(mapSize.MapHeight):
						for x in range(mapSize.MapWidth):
								plot = gameMap.plot(x,y)
								impType = plot.getImprovementType()
								# PAE WATCHTOWER to TURM
								if impType == GetInfoType("IMPROVEMENT_TURM"):
										count += 1
										plot.setImprovementType(GetInfoType("NO_IMPROVEMENT"))
...
##            print "placing towers in region %(r)d" % {"r":region.ID}
						if PRand.randint(0,1) == 0:
								...
								if chokePoint != None and not badNeighbor:
										# PAE: IMPROVEMENT_TOWER changed to IMPROVEMENT_TURM or IMPROVEMENT_TURM2
										if gc.getGame().getCurrentEra() > 1:
											chokePoint.setImprovementType(GetInfoType("IMPROVEMENT_TURM2"))
										else:
											chokePoint.setImprovementType(GetInfoType("IMPROVEMENT_TURM"))
...
##            print "midPoint.x= %(mx)d, midPoint.y= %(my)d" % {"mx":midPoint.x,"my":midPoint.y}
						...
						if bestPlot != None:
								...
								# PAE: IMPROVEMENT_TOWER changed to IMPROVEMENT_TURM or IMPROVEMENT_TURM2
								if gc.getGame().getCurrentEra() > 1:
									midHill.setImprovementType(GetInfoType("IMPROVEMENT_TURM2"))
								else:
									midHill.setImprovementType(GetInfoType("IMPROVEMENT_TURM"))
		...
		def createChokePointList(self):
				...
				for possibleChoke in possibleChokeList:
						for area in possibleChoke.neighborAreaList:
								if area.size > ChokePointAreaSize:
...
##                {"ix":inX,"iy":inY,"ox":outX,"oy":outY,"d":distance}
								if distance >= ChokePointWalkAroundDistance or distance == -1:
...
##                print "This secondary choke touches origional area and must be declared a dead end."
								return False
						elif area != origionalArea and area.size > ChokePointAreaSize:
								twoAreasFound = True
...
def getDescription():
				...
				"""
				return "TXT_KEY_MAP_SCRIPT_CREATION"
...
def addFeatures():
		...
		terrainSwamp = gc.getInfoTypeForString("TERRAIN_SWAMP")
		featureSavanna = gc.getInfoTypeForString("FEATURE_SAVANNA")
		featureDenseForest = gc.getInfoTypeForString("FEATURE_DICHTERWALD")
		FORESTLEAFY = 0
		FORESTEVERGREEN = 1
		FORESTSNOWY = 2
				...
				for x in range(mapSize.MapWidth):
						...
						plotMap.plotMap[i] != plotMap.PEAK:
								...
								if rainfall >= PRand.random():#Trees are present
										...
										if altitude < LeafyAltitude:
												...
												# Savanna for PAE
												elif altitude <= SavannaAltitude:
														plot.setFeatureType(featureSavanna,0)
												else:
														plot.setFeatureType(featureForest,FORESTLEAFY)
										elif altitude < EvergreenAltitude:
												plot.setFeatureType(featureForest,FORESTLEAFY)
										# Dense Forest for PAE
										elif altitude < DenseForestAltitude:
												plot.setFeatureType(featureDenseForest,0)
										elif altitude < TundraThreshold:
						...
						plotMap.plotMap[i] != plotMap.PEAK and plot.isWater() == False:
								if plot.isRiver() == True:
										# PAE set Plains too in desert rivers
										if PRand.randint(0,1) < 1:
											plot.setFeatureType(featureFloodPlains,0)
										else:
											plot.setTerrainType(terrainMap.PLAINS,0,0)
								else:
						...
						# Swamp for PAE -> 25% to a river
						if terrainMap.terrainMap[i] == terrainMap.GRASS and plotMap.plotMap[i] != plotMap.PEAK and \
						plotMap.plotMap[i] != plotMap.HILLS and not plot.isWater():
								if plot.isRiver(): iRand = 5
								else: iRand = 10
								if PRand.randint(0,iRand) < 1:
									 plot.setTerrainType(terrainSwamp,0,0)
...
def afterGeneration():#This comes before starting plot assignment
		...
		# PAE: create Towers
		count = CyMap().getLandPlots() / 15
		if count > 0:#Might not be FFH, so dont try to place towers if not
				spf.replaceWatchtowers(count)
				#spf.replaceUniqueImprovements()  # PAE: deactivated or hill resources won't be placed!

		# PAE creates some more gates between peaks
		lPeaks = list()
		lPeaksCoast = list()
			...
			if i > 3 and i+3 < MapW:
				for j in range (MapH):
					if j > 3 and j+3 < MapH:
						...
						if pPlot.isPeak() and not pPlot.isRiver():
							pPlotN = CyMap().plot(i, j+1)
							pPlotS = CyMap().plot(i, j-1)
							pPlotW = CyMap().plot(i-1, j)
							pPlotE = CyMap().plot(i+1, j)
							if (pPlotN.isPeak() or pPlotN.isWater()) and (pPlotS.isPeak() or pPlotS.isWater()) and not pPlotW.isPeak() and not pPlotE.isPeak() and not pPlotW.isWater() and not pPlotE.isWater() or \
								 (pPlotW.isPeak() or pPlotW.isWater()) and (pPlotE.isPeak() or pPlotE.isWater()) and not pPlotN.isPeak() and not pPlotS.isPeak() and not pPlotN.isWater() and not pPlotS.isWater():
								lPeaks.append(pPlot)
							# Coast borders
							if pPlotN.isPeak() and pPlotS.isWater() or pPlotS.isPeak() and pPlotN.isWater() or pPlotW.isPeak() and pPlotE.isWater() or pPlotE.isPeak() and pPlotW.isWater():
								lPeaksCoast.append(pPlot)
		
		# Coast peaks
		if len(lPeaksCoast):
			for p in lPeaksCoast:
				p.setTerrainType(GetInfoType("TERRAIN_HILL"),1,1)
				p.setTerrainType(GetInfoType("TERRAIN_PLAINS"),1,1)
		...
		iGates = sumPeaks / 2
		if iGates > 0:
			for i in range(iGates):
				iRand = PRand.randint(0,sumPeaks-1)

				lPeaks[iRand].setTerrainType(GetInfoType("TERRAIN_HILL"),1,1)
				lPeaks[iRand].setTerrainType(GetInfoType("TERRAIN_PLAINS"),1,1)
				lPeaks[iRand].setPlotType(PlotTypes.PLOT_HILLS,True,True)
